...
public class TimetableService {
    ...
    // Required for student's own timetable
    ...
    public TimetableResponse createTimetableEntry(
        ...
        TimeSlot timeSlot =
                timeSlotRepository
                        .findById(request.getTimeSlotId())
                        .orElseThrow(() ->
                                new ResourceNotFoundException(
                                        "Time slot not found"
                                ));
        ...
        Long teacherId =
                subjectOffering.getTeacher().getId();

        Long divisionId =
                subjectOffering.getDivision().getId();
    ...
    private void checkTeacherConflict(
            TimetableRequest request,
            Long teacherId) {
    ...
    private void checkDivisionConflict(
            TimetableRequest request,
            Long divisionId) {
    ...
    public List<TimetableResponse> getByTeacher(
            Long teacherId) {

        return timetableRepository
                .findBySubjectOfferingTeacherId(teacherId)
                .stream()
                .map(this::toResponse)
    ...
    /**
     * Returns the timetable for the currently
    ...
    public List<TimetableResponse> getByDivision(
            Long divisionId) {

        return timetableRepository
                .findBySubjectOfferingDivisionId(divisionId)
                .stream()
                .map(this::toResponse)
                .collect(Collectors.toList());
    }
    ...
    private TimetableResponse toResponse(
        ...
        SubjectOffering offering =
                timetable.getSubjectOffering();

        return new TimetableResponse(
                timetable.getId(),
                offering.getId(),
                offering.getSubject().getName(),
                offering.getTeacher().getFirstName()
                        + " "
                        + offering.getTeacher().getLastName(),
